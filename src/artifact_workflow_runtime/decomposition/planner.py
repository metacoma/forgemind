...
def _runtime_facts(snapshot: WorkflowStateSnapshot | None) -> dict[str, object]:
    ...
    workspace_snapshot = getattr(snapshot, "workspace_snapshot", None) or {}
    if isinstance(workspace_snapshot, dict):
        mutation_scope.extend(str(item) for item in workspace_snapshot.get("changed_paths", []) if str(item).strip())
