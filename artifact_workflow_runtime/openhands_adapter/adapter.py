...
def _verification_passed_from_bundle(*, text: str, ok: bool, transport_error: bool, bundle: EvidenceBundle) -> bool:
    if not ok or transport_error or bundle.structured.blockers:
        return False
...
def _schema_for_model(model: type) -> tuple[dict[str, Any], dict[str, Any]]:
    """Return a JSON Schema fragment plus definitions for a Pydantic model."""
...
def _array_schema_for_model(model: type) -> tuple[dict[str, Any], dict[str, Any]]:
    item_schema, defs = _schema_for_model(model)
    return {"type": "array", "items": item_schema, "default": []}, defs
...
def _field_json_schema(field_name: str) -> tuple[dict[str, Any], dict[str, Any]]:
    """Schema fragment for known runtime handoff fields.

    Unknown fields are intentionally allowed only as simple JSON values so the
    model is not encouraged to invent nested controller state. Known operational
    evidence fields use the same Pydantic models the runtime validates after the
    follow-up response is collected.
    """

    if field_name == "summary":
        return {"type": "string", "description": "Concise factual stage summary derived only from the completed conversation."}, {}
    if field_name == "structured_evidence":
        return _schema_for_model(StructuredEvidence)
    if field_name == "commands_run":
        return _array_schema_for_model(CommandEvidence)
    if field_name in {"files_changed", "files_observed"}:
        return _array_schema_for_model(FileEvidence)
    if field_name in {"extracted_facts", "facts"}:
        return _array_schema_for_model(ExtractedFact)
    if field_name == "diffs":
        return _array_schema_for_model(DiffEvidence)
    if field_name in {"tests", "checks"}:
        return _array_schema_for_model(TestCheckEvidence)
    if field_name == "blockers":
        return _array_schema_for_model(BlockerEvidence)
    if field_name == "mutation_summary":
        return _schema_for_model(MutationSummary)
    if field_name == "postcheck_summary":
        return _schema_for_model(PostcheckSummary)
    if field_name == "repair_summary":
        return {"type": "string", "description": "Concise factual repair summary."}, {}
    if field_name == "passed":
        return {"type": "boolean", "description": "True only when the existing evidence proves the requested check passed."}, {}
    if field_name == "confidence":
        return {"type": "string", "enum": ["low", "medium", "high"], "description": "Confidence based only on available evidence."}, {}
    if field_name in {"missing_evidence", "checks_passed", "checks_failed"}:
        return _string_array_schema(f"List of {field_name.replace('_', ' ')} items from existing evidence."), {}
    if field_name == "completion_status":
        return {
            "type": "string",
            "enum": ["completed", "implemented_not_verified", "verified_not_published", "partially_completed", "blocked"],
            "description": "Completion status supported by existing evidence only.",
        }, {}
    if field_name == "valid_json":
        return {"type": "boolean"}, {}
    return {
        "description": f"Value for declared contract field {field_name!r}. Use null if unsupported by existing evidence.",
        "type": ["string", "number", "boolean", "array", "object", "null"],
    }, {}


def _json_schema_for_response_contract(response_contract: StructuredResponseContract) -> str:
    properties: dict[str, Any] = {}
    required: list[str] = []
    defs: dict[str, Any] = {}

    for field in response_contract.required_fields:
        schema, field_defs = _field_json_schema(field.name)
...
def _contract_repair_prompt(*, stage: str, response_contract: StructuredResponseContract, evidence_requirements: object | None) -> str:
    ...
    lines = [
        ...
        "Do not infer new facts. Do not fill gaps with likely values. Put gaps in blockers or missing_evidence.",
        "For every non-empty commands_run, files_observed, files_changed, tests, diffs, or extracted_facts item, use concrete evidence already visible earlier in this conversation.",
...
class OpenHandsAdapter:
    ...
    async def _run_json_handoff_followup(self, *, stage: str, request: object, run) -> object | None:
        ...
        return await followup(conversation=run, prompt=repair_prompt)
    ...
    async def verify(self, request: VerificationRequest) -> VerificationResult:
        ...
        if effective_stage_failure is not None and effective_stage_failure.failure_kind == StageFailureKind.EVIDENCE_CONTRACT_MISSING:
