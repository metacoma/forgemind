...
def _clear_for_reentry(target: str) -> dict[str, Any]:
    ...
    if target in {"research", "observe", "build_context", "done_contract", "obligations"}:
        common.update({"obligations": None, "plan": None, "acceptance_contract": None})
    if target in {"done_contract", "obligations", "plan"}:
        common.update({"plan": None, "acceptance_contract": None})
    if target in {"research", "observe", "build_context"}:
        common.update({"context_packet": None, "done_contract": None, "environment_plan": None, "workspace_branch": None})
    elif target == "done_contract":
        common.update({"done_contract": None, "environment_plan": None, "workspace_branch": None})
    return common
