...
class ContractPrepStageMixin:
    async def done_contract_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        readiness_gate.require(state, "done_contract", "task", "classification", "context_packet")
        ...
        await _emit(services, "stage_started", "done_contract", "Compiling done contract from task and observed context", task_id=task.id)
        ...
        return {
            ...
            "transitions": _append_transition(state, "done_contract", "done_contract_built", "Done contract compiled from task and observed context before planning.", [artifact.id]),
        }
