...
def wire_workflow_graph(graph: Any, *, nodes: Mapping[str, NodeFn], routers: Mapping[str, RouteFn], end: object) -> Any:
    ...
    graph.add_edge("build_context", "done_contract")
    graph.add_edge("done_contract", "obligations")
    ...
    graph.add_conditional_edges("qa_review", routers["qa_review"], {"acceptance": "acceptance", "repair": "repair", "finalize": "finalize", "research": "research", "observe": "observe", "build_context": "build_context", "done_contract": "done_contract", "obligations": "obligations", "plan": "plan"})
    graph.add_edge("repair", "review")
    graph.add_conditional_edges("acceptance", routers["acceptance"], {"publish": "publish", "finalize": "finalize", "research": "research", "observe": "observe", "build_context": "build_context", "done_contract": "done_contract", "obligations": "obligations", "plan": "plan"})
