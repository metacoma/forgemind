...
`lifecycle.OpaPolicyEvaluator` evaluates publish/finalize/execute-exit gates through `src/artifact_workflow_runtime/lifecycle/policies/runtime.rego`. Policy fallback is now explicit: `mode="opa_required"` fails closed when OPA cannot return a decision, while `mode="dev_fallback"` keeps local tests/offline development runnable with the in-process evaluator. Production wiring should use `opa_required`.

### LangGraph

`graph.workflow` is the graph composition root: it wires the executor topology and delegates node behavior to `stages.WorkflowStageNodes`. It is the runtime/orchestration layer, not the reasoning backend and not the policy engine.

Stage behavior is now physically grouped under `stages/` by pipeline concern: intake/routing, observation/context, planning/policy, execution/repair, publishing, and verification/acceptance. `stages.workflow_nodes.WorkflowStageNodes` is only a small composed facade over those mixins.

When the optional `langgraph` dependency is unavailable, `graph.compat` provides a minimal async state graph for tests and local development. Production wiring should use real LangGraph plus the runtime checkpoint recorder.

### Runtime checkpoints

Every successful graph node can be wrapped by `state.WorkflowCheckpointRecorder`. The recorder persists a `workflow_checkpoint` artifact after each stage with the merged stage state, previous status, new status, and updated keys. These checkpoints are deliberately not appended to public `artifact_ids`; they are runtime/debug/resume artifacts. A LangGraph-native checkpointer can later replace or complement this recorder without changing stage implementations.
...
### Action ACL / PDP / PEP seed

The policy package now contains a typed action authorization seed: `RuntimeAction`, `RuntimeSubject`, `RuntimeResource`, `PolicyDecisionPoint`, and `PolicyEnforcementPoint`. The first backend is `StaticStagePolicyDecisionPoint`, which is fail-closed for known side effects. It denies publish-only actions such as `git.push` and `pr.create` outside the publish stage and authorizes stage-local actions before OpenHands packets are dispatched. This is still stage-level enforcement, not full tool-call interception, but it replaces purely prompt-level action boundaries with a concrete PDP/PEP interface.
...
- Added reference Rego policy in `lifecycle/policies/runtime.rego`; Python fallback is now an explicit development mode, while production policy can fail closed with `opa_required`.
- Removed the misleading `python-statemachine` dependency; lifecycle is a plain policy-backed transition engine with explicit OPA-required or dev-fallback policy modes.
