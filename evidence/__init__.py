...
_EVIDENCE_KEYS = {"commands_run", "files_changed", "files_observed", "extracted_facts", "facts", "diffs", "tests", "checks", "blockers", "mutation_summary", "postcheck_summary"}
...
class EvidenceExtractor:
    ...
    def _from_json_contract(self, text: str, *, artifact_id: str | None = None) -> StructuredEvidence | None:
        ...
        # OpenHands repair follow-ups commonly return a top-level JSON object
        # shaped like {summary, structured_evidence, blockers, unknowns}.  The
        # operational sections may be split between the nested
        # structured_evidence object and top-level fields.  Treat both as one
        # evidence document instead of rejecting a valid handoff just because
        # facts are grouped under structured_evidence.facts.
        if isinstance(payload.get("structured_evidence"), Mapping):
            data = dict(payload["structured_evidence"])
            for key in _EVIDENCE_KEYS:
                if key not in data and key in payload:
                    data[key] = payload[key]
        else:
            data = dict(payload)

        if not (_EVIDENCE_KEYS & set(data.keys())):
            return None
        artifact_ids = [artifact_id] if artifact_id else []
        try:
            evidence = StructuredEvidence(
                ...
                extracted_facts=self._facts(data.get("extracted_facts"), data.get("facts"), artifact_ids),
    ...
    def _command(self, item: object, artifact_ids: list[str]) -> CommandEvidence:
        if isinstance(item, Mapping):
            ...
            if "output_excerpt" not in data and "summary" in data:
                data["output_excerpt"] = data.get("summary")
            data.setdefault("output_artifact_ids", artifact_ids)
            allowed = {"command", "cwd", "exit_code", "output_excerpt", "output_artifact_ids"}
            return CommandEvidence.model_validate({key: value for key, value in data.items() if key in allowed})
    ...
    def _facts(self, extracted: object, facts: object, artifact_ids: list[str]) -> list[ExtractedFact]:
        items: list[ExtractedFact] = []
        for item in _list(extracted):
            items.append(self._fact(item, artifact_ids))
        if facts is not None:
            if isinstance(facts, Mapping) and "fact" not in facts:
                for subject, value in facts.items():
                    items.append(self._fact_mapping_entry(str(subject), value, artifact_ids))
            else:
                for item in _list(facts):
                    items.append(self._fact(item, artifact_ids))
        return items[: self.max_items]

    def _fact_mapping_entry(self, subject: str, value: object, artifact_ids: list[str]) -> ExtractedFact:
        if isinstance(value, (Mapping, list)):
            try:
                fact = json.dumps(value, ensure_ascii=False, sort_keys=True)
            except TypeError:
                fact = str(value)
        else:
            fact = str(value)
