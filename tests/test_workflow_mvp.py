...
async def test_html_execution_evidence_fails_verification_without_second_openhands_run(tmp_path) -> None:
    ...
    assert report.execution.stage_failure.failure_kind.value == "html_transport_error"
...
async def test_executor_pr_side_effect_stops_before_publish(tmp_path) -> None:
    artifact_store = ArtifactStore(tmp_path / "artifacts")
    llm = ScriptedLLMBackend(
        {
            "classification": [
            ...
            "obligation_analysis": [
                {
                    ...
                    "required_publish_actions": ["create_pr"],
                    "completion_requirements": ["feature implemented", "PR exists"],
                    "blocker_conditions": [],
                    "reasoning_summary": "PR creation is a publish obligation, not an execute obligation.",
                }
            ],
            "planning": [
                {
                    "summary": "Implement the change; publish stage must open the PR",
                    "execution_family": ExecutionFamily.REPOSITORY_CHANGE.value,
                    "task_intent": "implement",
                    "deliverable_kind": "repository_changes",
                    "capabilities": [Capability.REPO_WRITE.value, Capability.REPO_CREATE_PR.value],
                    "steps": ["inspect repo", "implement", "run unit tests"],
                    "success_criteria": ["feature implemented", "unit tests pass", "PR exists"],
                    ...
                    "require_commit": False,
                    "require_push": False,
                    "reasoning": "Executor must not create the PR; publish stage owns PR side effects.",
                }
            ],
        }
    )
    openhands = FakeOpenHandsAdapter(
        ...
        scripts={
            "observe": ["Repository observed."],
            "execute": [
                '{"summary":"Implemented code change and tests passed.","structured_evidence":{'
                '"commands_run":[{"command":"pytest tests/test_feature.py","exit_code":0,"output_excerpt":"1 passed"},'
                '{"command":"gh pr create --fill --base main","exit_code":0,"output_excerpt":"https://github.com/acme/repo/pull/42"}],'
                '"files_changed":[{"path":"src/app.py","action":"changed","summary":"Implemented change"}],'
                '"tests":[{"name":"pytest tests/test_feature.py","command":"pytest tests/test_feature.py","status":"passed","passed":true}],'
                '"blockers":[],'
                '"mutation_summary":{"changed":true,"summary":"Implemented change","files_changed":["src/app.py"]},'
    ...
    assert report.status == "control_plane_violation"
    assert report.publish is None
    assert len(openhands.calls["execute"]) == 1
    assert len(openhands.calls["publish"]) == 0
